...
from reg_utils import  train_reg1
from reg_utils import  evaluate
###########################parse inputs#######################################
import os
import argparse
##############################################################################
parser = argparse.ArgumentParser(description='CIFAR-10 SAT-R1')
parser.add_argument('--exp_name',default='sat_r1_cifar10', type=str, help='Name of the experiment')
...
parser.add_argument('--model_weight_prefix',default='models/cifar10_reg1_epoch_',type=str, help='Path to save models')
parser.add_argument('--train_log_fname',default='log/train_loss_sat_reg1.txt',type=str, help='Train loss log file name')
...
parser.add_argument('--lambda',default=0.2, type=float, help='lambda')
parser.add_argument('--epsilon', default =8.0/255, type=float, help='Epsilon')
parser.add_argument('--train_ifgsm_steps', default =3, type=int, help='Train IFGSM steps')
...
def main():
    ...
    model.train()
    model.cuda()
    train_reg1(model,train_loader,config)
